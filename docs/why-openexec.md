...
Sovereignty is the power to choose who sees your data. OpenExec v0.1.10 allows you to use powerful cloud models for thinking, while keeping the actual data processing and sensitive details local.
...
*   **PID Tracking:** v0.1.10 introduces **Automated PID Management**. It writes a process file to `.openexec/openexec.pid` and ensures that only one engine is active per project.
...
*   **Automatic Port Probing:** If the default port (8080) is busy (common on developer machines), OpenExec v0.1.10 automatically scans for the next available port, ensuring the system always starts without manual configuration.
